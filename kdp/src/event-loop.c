...
static int catch_errors(int a, int b, int c, int d) {
	printf("(catch-errors)");
}
...
static struct
  {
    gdb_event2 *first_event;	/* First pending event */
    gdb_event2 *last_event;	/* Last pending event */
  }
event_queue;
...
void async_queue_event (gdb_event2 * event_ptr, queue_position position);
int sigint_taken_p(void);
static gdb_event2 *create_file_event (int fd);
...
void
async_queue_event (gdb_event2 * event_ptr, queue_position position)
{
  if (position == TAIL)
    {
...
{
  gdb_event2 *file_event_ptr;

  file_event_ptr = (gdb_event2 *) malloc (sizeof (gdb_event2));
...
create_file_handler (int fd, int mask, handler_func * proc, gdb_client_data client_data)
{
  ...
  if (file_ptr == NULL)
    {
      file_ptr = (file_handler *) malloc (sizeof (file_handler));
      file_ptr->fd = fd;
  ...
  if (use_poll)
    {
      ...
      new_poll_fds =
	(struct pollfd *) malloc ((gdb_notifier.num_fds - 1) * sizeof (struct pollfd));
  ...
  for (file_ptr = gdb_notifier.first_file_handler; file_ptr != NULL;
      ...
      if (file_ptr->fd == event_file_desc)
	{
	  ...
	  if (use_poll)
		...
		{
		  printf ("Exception condition detected on fd %d\n", file_ptr->fd);
		  file_ptr->error = 1;
		}
	      else
		file_ptr->error = 0;
...
create_async_signal_handler (sig_handler_func * proc, gdb_client_data client_data)
{
  ...
  async_handler_ptr =
    (async_signal_handler *) malloc (sizeof (async_signal_handler));
...
create_timer (int milliseconds, timer_handler_func * proc, gdb_client_data client_data)
{
  ...
  timer_ptr = (struct gdb_timer *) malloc (sizeof (gdb_timer));
...
poll_timers (void)
{
  ...
  if (timer_list.first_timer != NULL)
	...
	{
	  event_ptr = (gdb_event2 *) malloc (sizeof (gdb_event2));
