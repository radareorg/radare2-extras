...
{
  fputs ("Argument required (target name).  Try `help target'\n",
		  stderr);
}
  ...
  if (!target_structs)
    {
      ...
      target_structs = (struct target_ops **) malloc
...
push_target (struct target_ops *t)
{
  ...
  tmp = (struct target_stack_item *)
    malloc (sizeof (struct target_stack_item));
  tmp->next = cur;
  tmp->target_ops = t;
...
target_resize_to_sections (struct target_ops *target, int num_added)
{
  ...
  if (target->to_sections)
  ...
  else
    {
      old_count = 0;
      target->to_sections = (struct section_table *)
	malloc ((sizeof (struct section_table)) * num_added);
    }
...
debug_to_open (char *args, int from_tty)
{
  ...
  fprintf(stderr, "target_open (%s, %d)\n", args, from_tty);
}
...
debug_to_attach (char *args, int from_tty)
{
  ...
  fprintf (stderr, "target_attach (%s, %d)\n", args, from_tty);
}
...
debug_to_post_attach (int pid)
{
  ...
  fprintf (stderr, "target_post_attach (%d)\n", pid);
}
...
debug_to_detach (char *args, int from_tty)
{
  ...
  fprintf (stderr, "target_detach (%s, %d)\n", args, from_tty);
}
...
debug_to_resume (int pid, int step, enum target_signal siggnal)
{
  ...
  fprintf (stderr, "target_resume (%d, %s, %s)\n", pid,
		      step ? "step" : "continue",
...
debug_to_wait (int pid, struct target_waitstatus *status)
{
  ...
  switch (status->kind)
    ...
    case TARGET_WAITKIND_STOPPED:
      fprintf (stderr, "stopped, signal = %s\n",
			  target_signal_to_name (status->value.sig));
      break;
    case TARGET_WAITKIND_SIGNALLED:
      fprintf (stderr, "signalled, signal = %s\n",
			  target_signal_to_name (status->value.sig));
      break;
...
debug_to_post_wait (int pid, int status)
{
  ...
  fprintf (stderr, "target_post_wait (%d, %d)\n",
		      pid, status);
}

static const char *REGISTER_NAME(int regno) {
	return "??";
}
...
debug_to_fetch_registers (int regno)
{
  ...
  fprintf (stderr, "target_fetch_registers (%s)",
...
debug_to_store_registers (int regno)
{
  ...
  if (regno >= 0 && regno < NUM_REGS)
    fprintf (stderr, "target_store_registers (%s) = 0x%lx %ld\n",
			REGISTER_NAME (regno),
			(unsigned long) read_register (regno),
			(unsigned long) read_register (regno));
  else
    fprintf (stderr, "target_store_registers (%d)\n", regno);
}
...
debug_to_prepare_to_store (void)
{
  ...
  fprintf (stderr, "target_prepare_to_store ()\n");
}
  ...
  if (retval > 0)
      ...
      for (i = 0; i < retval; i++)
	{
	  if ((((long) &(myaddr[i])) & 0xf) == 0)
	    fprintf (stderr, "\n");
	  fprintf (stderr, " %02x", myaddr[i] & 0xff);
	}
    }
...
debug_to_files_info (struct target_ops *target)
{
  ...
  fprintf (stderr, "target_files_info (xxx)\n");
}
...
debug_to_terminal_init (void)
{
  ...
  fprintf (stderr, "target_terminal_init ()\n");
}
...
debug_to_terminal_inferior (void)
{
  ...
  fprintf (stderr, "target_terminal_inferior ()\n");
}
...
debug_to_terminal_ours_for_output (void)
{
  ...
  fprintf (stderr, "target_terminal_ours_for_output ()\n");
}
...
debug_to_terminal_ours (void)
{
  ...
  fprintf (stderr, "target_terminal_ours ()\n");
}
...
debug_to_terminal_info (char *arg, int from_tty)
{
  ...
  fprintf (stderr, "target_terminal_info (%s, %d)\n", arg,
		      from_tty);
}
...
debug_to_load (char *args, int from_tty)
{
  ...
  fprintf (stderr, "target_load (%s, %d)\n", args, from_tty);
}
...
debug_to_lookup_symbol (char *name, CORE_ADDR *addrp)
{
  ...
  fprintf (stderr, "target_lookup_symbol (%s, xxx)\n", name);
...
debug_to_create_inferior (char *exec_file, char *args, char **env)
{
  ...
  fprintf (stderr, "target_create_inferior (%s, %s, xxx)\n",
		      exec_file, args);
}
...
debug_to_post_startup_inferior (int pid)
{
  ...
  fprintf (stderr, "target_post_startup_inferior (%d)\n",
		      pid);
}
...
debug_to_acknowledge_created_inferior (int pid)
{
  ...
  fprintf (stderr, "target_acknowledge_created_inferior (%d)\n",
		      pid);
}
...
debug_to_post_follow_inferior_by_clone (void)
{
  ...
  fprintf (stderr, "target_post_follow_inferior_by_clone ()\n");
}
...
debug_to_insert_fork_catchpoint (int pid)
{
  ...
  fprintf (stderr, "target_insert_fork_catchpoint (%d) = %d\n",
		      pid, retval);
...
debug_to_remove_fork_catchpoint (int pid)
{
  ...
  fprintf (stderr, "target_remove_fork_catchpoint (%d) = %d\n",
		      pid, retval);
...
debug_to_insert_vfork_catchpoint (int pid)
{
  ...
  fprintf (stderr, "target_insert_vfork_catchpoint (%d)= %d\n",
		      pid, retval);
...
debug_to_remove_vfork_catchpoint (int pid)
{
  ...
  fprintf (stderr, "target_remove_vfork_catchpoint (%d) = %d\n",
		      pid, retval);
...
debug_to_has_forked (int pid, int *child_pid)
{
  ...
  fprintf (stderr, "target_has_forked (%d, %d) = %d\n",
...
debug_to_has_vforked (int pid, int *child_pid)
{
  ...
  fprintf (stderr, "target_has_vforked (%d, %d) = %d\n",
...
debug_to_can_follow_vfork_prior_to_exec (void)
{
  ...
  fprintf (stderr, "target_can_follow_vfork_prior_to_exec () = %d\n",
...
debug_to_insert_exec_catchpoint (int pid)
{
  ...
  fprintf (stderr, "target_insert_exec_catchpoint (%d) = %d\n",
		      pid, retval);
...
debug_to_remove_exec_catchpoint (int pid)
{
  ...
  fprintf (stderr, "target_remove_exec_catchpoint (%d) = %d\n",
		      pid, retval);
...
debug_to_has_execd (int pid, char **execd_pathname)
{
  ...
  fprintf (stderr, "target_has_execd (%d, %s) = %d\n",
...
debug_to_has_exited (int pid, int wait_status, int *exit_status)
{
  ...
  fprintf (stderr, "target_has_exited (%d, %d, %d) = %d\n",
...
debug_to_mourn_inferior (void)
{
  ...
  fprintf (stderr, "target_mourn_inferior ()\n");
}
...
debug_to_can_run (void)
{
  ...
  fprintf (stderr, "target_can_run () = %d\n", retval);
...
debug_to_notice_signals (int pid)
{
  ...
  fprintf (stderr, "target_notice_signals (%d)\n", pid);
}
...
debug_to_thread_alive (int pid)
{
  ...
  fprintf (stderr, "target_thread_alive (%d) = %d\n",
		      pid, retval);
     ...
     int pid;
{
  ...
  fprintf (stderr, "target_thread_alive (%d) = %s\n", pid, retval);
...
debug_to_query (int type, char *req, char *resp, int *siz)
{
  ...
  fprintf (stderr, "target_query (%c, %s, %s,  %d) = %d\n", type, req, resp, *siz, retval);
...
debug_to_get_current_exception_event (void)
{
  ...
  fprintf (stderr, "target get_current_exception_event ()\n");
  return result;
}
...
debug_to_pid_to_exec_file (int pid)
{
  ...
  fprintf (stderr, "target_pid_to_exec_file (%d) = %s\n",
...
debug_to_core_file_to_sym_file (char *core)
{
  ...
  fprintf (stderr, "target_core_file_to_sym_file (%s) = %s\n",
		      core, sym_file);
