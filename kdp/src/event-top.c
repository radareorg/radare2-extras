...
#include "event-top.h"
int saved_command_line = 0;
int saved_command_line_size = 0;
FILE *instream = NULL;
FILE *gdb_stdin = NULL;
extern int quit_flag;
int batch_silent = 0;
FILE *gdb_stdout = NULL;
FILE *gdb_stderr = NULL;
int annotation_level = 0;
int immediate_quit = 0;
int history_expansion_p = 0;
...
display_gdb_prompt (const char *new_prompt)
{
  ...
  if (! new_prompt)
    ...
    actual_gdb_prompt = strdup (new_prompt);
...
top_level_prompt (void)
{
  ...
  composed_prompt = malloc (prompt_length + 1);
	  ...
	  if (strlen (history_value) > linelength)
	    {
	      ...
	      linebuffer = (char *) realloc (linebuffer, linelength);
	    }
