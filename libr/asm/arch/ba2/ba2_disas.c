...
static int ba2_decode_opcode(ut64 pc, const ut8 *bytes, int len, struct op_cmd *cmd, RStrBuf *esil, RAnalOp *anal) {
	if (0) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,=", g, d);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,r%d,=", g, d, d);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,=", a, d);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_MOV;
		}
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,+,r%d,=", a, d, d);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_ADD;
		}
		...
		if (esil) {
			r_strbuf_appendf (esil, "2,0x%x,-,pc,+=", t);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[1]", b, n, a);
		}
		if (anal) {
			//???			anal->type = R_ANAL_OP_TYPE_ADD;
		}
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[1],r%d,=", n, a, d);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[2]", b, 2 * m, a);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[2],r%d,=", d, 2 * m, a);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[4]", b, k, a);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[4],r%d,=", k, a, d);
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,r%d,=", o, a, d);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_ADD;
		}
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,&,r%d,=", n, a, d);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0,fl,=,r%d,0x%x,==,$z,?{{,1,fl,}}", a, o);
		}
		return 3;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, 0x%x", a, o);
		if (esil) {
			r_strbuf_appendf (esil, "0,fl,=,0x%x,r%d,>,?{,1,fl,}", o, a);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0,fl,=,r%d,r%d,==,$z,?{,1,fl,}", b, a);
		}
		return 3;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, r%d", a, b);
		if (esil) {
			r_strbuf_appendf (esil, "0,fl,=,r%d,r%d,>=,?{,1,fl,}", b, a);
		}
		return 3;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, r%d", d, a);
		if (esil) {
			r_strbuf_appendf (esil, "0xff,r%d,&,r%d,=,", a, d);
		}
		return 3;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, r%d", d, a);
		if (esil) {
			r_strbuf_appendf (esil, "0,r%d,=,r%d,0x80,&,0x80,==,?{,0xffffff00,r%d,=,},0xff,r%d,&,r%d,|=,", d, a, d, a, d);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "%d,r%d,==,$z,?{,3,%d,-,pc,+=,}", e, b, p);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "pc,lr,=,r%d,pc,=", a);
		}
		if (anal) {
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d", a);
		if (esil) {
			r_strbuf_appendf (esil, "r%d,pc,=", a);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_UJMP;
		}
		...
		if (esil) {
			r_strbuf_appendf (esil, "fl,0,==,?{,3,0x%x,-,pc,+=,}", s);
		}
		return 3;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "0x%x", (unsigned int)(pc + s));
		if (esil) {
			r_strbuf_appendf (esil, "pc,lr,=,3,0x%x,-,pc,+=", s);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,&,r%d,=", b, a, d);
		}
		if (anal) {
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, r%d, r%d", d, a, b);
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,=,fl,1,==,?{,r%d,r%d,=,}", b, d, a, d);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,<<,r%d,=", h, a, d);
		}
		return 3;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[1]", b, h, a);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[1],r%d,=", h, a, d);
		}
		if (anal) {
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[2]", b, i * 2, a);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_STORE;
			//			anal->dst = r_anal_value_new ();
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[2],r%d,=", i * 2, a, d);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_LOAD;
			//			anal->dst = r_anal_value_new ();
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[4]", b, w * 4, a);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_STORE;
			//			anal->dst = r_anal_value_new ();
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[4],r%d,=", w * 4, a, d);
		}
		if (anal) {
			//			anal->type = R_ANAL_OP_TYPE_MOV;
			anal->type = R_ANAL_OP_TYPE_LOAD;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,r%d,=", g, a, d);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_ADD;
			if (a == 0) { // r0 is always 0
		...
		if (esil) {
			r_strbuf_appendf (esil, "0,fl,=,r%d,0x%x,==,$z,?{{,1,fl,}}", a, g);
		}
		return 6;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, 0x%x", a, g);
		if (esil) {
			r_strbuf_appendf (esil, "0,fl,=,0x%x,r%d,>,?{,1,fl,}", g, a);
		}
		return 6;
...
*/
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,==,$z,?{,6,0x%x,-,pc,+=,}", i, b, u);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_CJMP;
			anal->jump = pc + u;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,==,$z,?{,6,0x%x,-,pc,+=,}", b, a, u);
		}
		return 6;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, r%d, 0x%x", d, a, o);
		if (esil) {
			r_strbuf_appendf (esil, "0xDEADBEEF,r%d,=", d);
		} // TODO: MFSPR
		return 6;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,=", b, b);
		} // TODO: MTSPR
		return 6;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[1]", b, y, a);
		}
		return 4;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[1],r%d,=", y, a, d);
		}
		return 4;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[2],r%d,=", x, a, d);
		}
		return 4;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,0x%x,r%d,+,=[4]", b, w, a);
		}
		if (len < 4) {
			snprintf (cmd->operands, sizeof (cmd->operands), "len=%d", len);
		}
		return 4;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,[4],r%d,=", w, a, d);
		}
		return 4;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, 0x%x, %x", b, i, (unsigned int)(pc + u));
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,==,$z,?{,4,0x%x,-,pc,+=,}", i, b, u);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_CJMP;
			anal->jump = pc + u;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, 0x%x, %x", b, i, (unsigned int)(pc + u));
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,>,?{,4,0x%x,-,pc,+=,}", i, b, u);
		}
		return 4;
		...
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,==,$z,?{,4,0x%x,-,pc,+=,}", b, a, u);
		}
		return 4;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "r%d, r%d, %x", a, b, (unsigned int)(pc + u));
		if (esil) {
			r_strbuf_appendf (esil, "r%d,r%d,>=,?{,4,0x%x,-,pc,+=,}", b, a, u);
		}
		return 4;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "%x", (unsigned int)(pc + t));
		if (esil) {
			r_strbuf_appendf (esil, "pc,lr,=,4,0x%x,-,pc,+=", t);
		}
		return 4;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "%x", (unsigned int)(pc + t));
		if (esil) {
			r_strbuf_appendf (esil, "fl,1,==,?{,4,0x%x,-,pc,+=,}", t);
		}
		return 4;
		...
		snprintf (cmd->operands, sizeof (cmd->operands), "%x", (unsigned int)(pc + t));
		if (esil) {
			r_strbuf_appendf (esil, "fl,0,==,?{,4,0x%x,-,pc,+=,}", t);
		}
		return 4;
		...
		if (esil) {
			r_strbuf_appendf (esil, "0x%x,r%d,+,r%d,=", y, a, d);
		}
		if (anal) {
			anal->type = R_ANAL_OP_TYPE_ADD;
		}
