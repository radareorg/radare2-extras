...
int r_asm_swf_disass(RBinObject *obj, char* buf_asm, const ut8* buf, int len, ut64 pc) {
	...
	if (isTag && len > 1) {
		...
		case SWFACTION_PUSH: {
			...
			while (i < len+2) {
				...
				case 1: { /* Floating point */
					float f = r_mem_get_num (buf+i, 4, 1);
					i += 4;
				...
				case 6: { /* Double */
					double d = r_mem_get_num (buf+i, 8, 1);
					sprintf (name, "double:%f", d);
					i += 8;
					break;
				}
				case 7: { /* Integer */
					int integer = r_mem_get_num (buf+i, 4, 1);
				...
				case 9: { /* Constant16 */
					ut16 c = r_mem_get_num (buf+i, 2, 1);
		...
		case SWFACTION_GOTOFRAME: {
			ut16 frame = r_mem_get_num (buf+1, 2, 1);
		...
		case SWFACTION_BRANCHIFTRUE: {
			short offset = r_mem_get_num (buf+1, 2, 1);
		...
		case SWFACTION_GOTOFRAME2: {
			...
			ut8 playFlag = buf[8];
			if (biasFlag == 1) {
				ut16 bias = r_mem_get_num (buf+9, 2, 1);
				dlen += 2;
			}
		...
		case SWFACTION_CONSTANTPOOL: {
			ut16 size = r_mem_get_num (buf+1, 2, 1);
			ut16 count = r_mem_get_num (buf+3, 2, 1);
		...
		case SWFACTION_DEFINEFUNCTION: {
			...
			ut16 nbParams = r_mem_get_num (buf+2, 2, 1);
			ut32 size = 0;
			ut16 i;
			...
			ut16 codeSize = r_mem_get_num (buf+4+i, 2, 1);
			break;
		}
